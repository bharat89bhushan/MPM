...
class DefaultController extends Controller
{
	...
	public function accessRules()
	{
		return array(
	 	array('allow',  // allow all users to perform 'index' and 'view' actions
			//			'actions'=>array('create1','upload','restore','index'),
						'users'=>array('*'),
		),
	...
	public function fileDownload($file = null)
	{
	...
	public function actionCreate()
	{
		...
		{
			if($tableName != 'sys_details' && $tableName != 'users')
		...
		$this->actionDelete($this->file_name);
	...
	public function actionClean($redirect = true)
	{
		...
		//	fwrite ( $this->fp, 'DROP TABLE IF EXISTS ' .addslashes($tableName) . ';'.PHP_EOL );
			fwrite ( $this->fp, 'truncate table ' .addslashes($tableName) . ';'.PHP_EOL );
...
//		$this->actionIndex();
	...
	public function funcRestore($file = null)
	{
		$this->updateMenuItems();
		$message = 'OK. Done';
		$sqlFile = $this->path . 'install.sql';
...
//		$this->render('restore',array('error'=>$message));
	...
	public function actionUpload()
	{
		...
		if(isset($_POST['UploadForm']))
		{
			...
			{
				$this->funcRestore($model->upload_file);
				$this->actionDelete($model->upload_file);
